...
class MetaFormer(nn.Module):
    ...
    def forward_features(self, x, x_176):
        pyramid = []
        # print(x.shape)
        # print(x_176.shape)
        if x_176 != None:
            pyramid.append(x)
            pyramid.append(x_176)
        for i in range(self.num_stage):
            # print(x.shape)
            # x_176 = self.downsample_layers[i](x_176)
            # x_176 = self.stages[i](x_176)
            # pyramid.append(x_176)
            x = self.downsample_layers[i](x)
            x = self.stages[i](x)
            pyramid.append(x)
            # print(x_176.shape)
        return pyramid  # (B, H, W, C) -> (B, C)

    def forward(self, x, x_176=None):
        pyramid = self.forward_features(x, x_176)
        # pyramid = self.forward_features(x)
        # x = self.head(x)
        # print(x.shape)
        return pyramid
